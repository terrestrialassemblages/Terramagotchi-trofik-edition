...
function createMicroAggregates(grid, testRadius, minBacteriaCount) {
    for (let y = 0; y < gridHeight; y++) {
        for (let x = 0; x < gridWidth; x++) {
            if (grid[y][x] === 'fungi') {
                const conditionsMet = checkMicroAggregateConditions(grid, x, y, testRadius, minBacteriaCount);
                if (conditionsMet) {
                    let end = Math.min(gridWidth - 1, x + 5);
...
// Function to allow micro-aggregates to grow into macro-aggregates
function growMicroAggregates() {
    for (let y = 0; y < gridHeight; y++) {
        for (let x = 0; x < gridWidth; x++) {
            if (grid[y][x] === 'aggregate') {
                /*const conditionsMet = checkMacroAggregateConditions(grid, x, y, testRadius);
                if (conditionsMet) {
                    grid[y][x] = 'aggregate';
                }*/
                if (grid[y][x - 1] === "soil") {
...
// Function to check conditions for macro-aggregate growth
/*function checkMacroAggregateConditions(grid, centerX, centerY, testRadius) {
    const minBoundX = Math.max(0, centerX - testRadius);
    const maxBoundX = Math.min(gridWidth - 1, centerX + testRadius);
    const minBoundY = Math.max(0, centerY - testRadius);
    const maxBoundY = Math.min(gridHeight - 1, centerY + testRadius);
    ...
    for (let y = minBoundY; y <= maxBoundY; y++) {
        for (let x = minBoundX; x <= maxBoundX; x++) {
            if (grid[y][x] === 'fungi') {
                rootCount++;
            } else if (grid[y][x] === 'root' || grid[y][x] === 'rootTip') {
                bacteriaCount++;
            }
        }
    }
    ...
    // Adjust these conditions as needed
    return bacteriaCount >= minBacteriaCount && rootCount === 0;
}*/
...
function loop() {
    ...
    const minBacteriaCount = 3;
    updateGrid();
    drawGrid();
    createMicroAggregates(grid, testRadiusMicro, minBacteriaCount);
    growMicroAggregates();
