...
function drawSandAutomatically() {
    const stoneMinSize = 3;
    const stoneMaxSize = 8;
    const stoneProbability = 0.04; //Control the probability of stone generation
    for (let y = gridHeight - 1; y >= gridHeight*0.6; y--) {
        for (let x = 0; x < gridWidth; x++) {
            if (grid[y][x] === null) {
                grid[y][x] = 'soil';
                // Generate stones within a specified height range
                if (y >= gridHeight * 0.6 + 7 && Math.random() < stoneProbability) {
                    const stoneSizeX = Math.floor(Math.random() * (stoneMaxSize - stoneMinSize + 1)) + stoneMinSize;
                    const stoneSizeY = Math.floor(Math.random() * (stoneMaxSize - stoneMinSize + 1)) + stoneMinSize;
...
function drawWaterAutomatically() {
    const waterProbability = 0.2;//Control the probability of water generation
    for (let y = gridHeight - 1; y >= gridHeight*0.6; y--) {
        for (let x = 0; x < gridWidth; x++) {
            if (grid[y][x] === 'soil') {
                if (y >= gridHeight * 0.6 + 2 && Math.random() < waterProbability) {
                    grid[y][x] = 'water';
                }
            }
        }
