...
function generateSoil(y, x, macro = false) {
    ...
    const soilColor = updateSoilColor(y, x, grid);
    console.log("New Soil Color:", soilColor);
    for (let i = y; i < y + 10; i++) {
        for (let j = x; j < x + 20; j++) {
            if (grid[i][j] === 'soil') {
                ...
                console.log("Drawing at:", j * cellSize, i * cellSize);
            }
        }
    }
}
...
function updateSoilColor(y, x, grid) {
    ...
    if (aggregateCount > 20) {
        return '#000000';
    } else if (aggregateCount > 10) {
        return '#a3551c';
    } else {
        return '#452c1b'; 
    }
