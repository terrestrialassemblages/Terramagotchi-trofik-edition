import { elements, canvas, globalY, cellSize } from '../sandSim.js';
...
export function updateSoilAlpha(y, x, aggrCount) {
    if (aggrCount >= 2) {
        let targetAlpha = elements.soil.initAlpha[y + "," + x] - (aggrCount - 2) * 0.05;
        targetAlpha = Math.min(Math.max(targetAlpha, 0), 0.7);  // Boundaries between 0 and 0.95
...
export function updateInitialAlpha(y, x, height) {
    ...
    let currY = globalY;
    let endY = Math.ceil(canvas.height/1.25/cellSize);


    // Normalize height to a 0-70 scale (150-80)
    height = height - currY;
    //console.log("height to scale", height);

    // Scale alpha so it's smaller when height is smaller
    let targetAlpha = 0.5 + height * (1 - 0.5) / (endY-globalY);
